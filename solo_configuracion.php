...
$token = (!empty($_POST['token_fmc']) )? $_POST['token_fmc']:((!empty($bd_usuarios[0]['token']))? $bd_usuarios[0]['token']:'') ;
//$token = (!empty($bd_usuarios[0]['token']))? $bd_usuarios[0]['token']:'' ;
$mensaje=array();
    ...
    if($usuario ==  $bd_usuarios[0]['usuario'] &&  $pass == $bd_usuarios[0]['password']){
        ...
        //descargamos los datos en la app para que los guarde en el movil
            ...
            if(!empty($UsuarioHorario)){
                $mensaje['mensaje']="Horarios descargados";
